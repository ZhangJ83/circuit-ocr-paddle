#!/bin/bash
# V4: fixed image_grid_thw + full sequence loss
LOGFILE=/mnt/g/mimo_project/circuit_ocr/circuit-ocr-dataset/training_run_v4.log
rm -f "$LOGFILE"
...
echo "" | tee -a "$LOGFILE"
echo "=== Internal training.log (tail 20) ===" | tee -a "$LOGFILE"
cat PaddleOCR-VL-LoRA-circuit-ocr/training.log 2>/dev/null | tail -20 | tee -a "$LOGFILE"
