#!/bin/bash
# Final light training: no mid-training saves, 32px + 64 tokens
LOGFILE=/mnt/g/mimo_project/circuit_ocr/circuit-ocr-dataset/training_final.log
rm -f "$LOGFILE"
...
python3 scripts/train_lora_light.py --rank 8 --alpha 16 --epochs 2 --max_label_len 100 --grad_accum 4 2>&1 | tee -a "$LOGFILE"
...
echo "" | tee -a "$LOGFILE"
echo "=== Checkpoints ===" | tee -a "$LOGFILE"
ls -la PaddleOCR-VL-LoRA-circuit-ocr/*.pdparams 2>/dev/null | tee -a "$LOGFILE"
echo "" | tee -a "$LOGFILE"
