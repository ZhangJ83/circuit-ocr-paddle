"""Projector LoRA r=16 NO-KV: remove k_proj/v_proj to force visual feature usage.
r=16, alpha=32, targets: q_proj + o_proj + Projector (linear_1, linear_2).
GQA 2 KV heads — LoRA on k/v enables shortcut learning. 3 epochs, 2433 samples.
"""
...
TARGETS = [
    ".*q_proj", ".*o_proj",        # self-attn query+output only
    ".*linear_1", ".*linear_2",    # Projector
]

log(f"Projector LoRA r=16 NO-KV: targets={TARGETS}")
...
save_path = f"{OUTPUT_DIR}/lora_projector_r16_nokv_fp16.pdparams"
