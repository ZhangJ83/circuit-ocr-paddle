"""Test: merge LoRA using in-place tensor addition (avoiding set_value/set_state_dict)."""
...
print("[1] Loading processor...", flush=True)
processor = AutoProcessor.from_pretrained(LOCAL_PATH)

print("[2] Loading base model...", flush=True)
...
# Test base forward first
print("[3] Testing base forward...", flush=True)
...
base_logits = out[0].numpy().copy()
print(f"[3] Base forward OK, logits={list(out[0].shape)}", flush=True)

# === In-place tensor addition approach ===
# Instead of set_value/set_state_dict, directly add delta to parameter tensor
print("[4] Loading LoRA and building param map...", flush=True)
lora_state = paddle.load(f"{LORA_DIR}/lora_weights_f32.pdparams")

# Build: LoRA base name -> (delta numpy)
lora_pairs = {}
...
print(f"[4] {len(lora_pairs)} LoRA pairs, {len(param_map)} model params", flush=True)

# Apply deltas via in-place addition
merged = 0
for lora_base, adapters in lora_pairs.items():
    ...
    if lora_base in param_map:
        param = param_map[lora_base]
        if list(delta.shape) == list(param.shape):
            # In-place addition using paddle.add with out parameter
            delta_tensor = paddle.to_tensor(delta.astype(np.float32), place=param.place)
            paddle.add(param, delta_tensor, out=param)  # In-place: param += delta
            merged += 1
            del delta_tensor

model.eval()
print(f"[4] Merged {merged}/{len(lora_pairs)} via paddle.add(out=param)", flush=True)

# Test forward after in-place merge
print("[5] Testing LoRA forward after in-place merge...", flush=True)
paddle.device.cuda.empty_cache()
try:
    with paddle.no_grad():
        out2 = model(input_ids=inputs["input_ids"], pixel_values=inputs.get("pixel_values"),
                     image_grid_thw=inputs.get("image_grid_thw"), use_cache=False)
    lora_logits = out2[0].numpy()
    diff = np.abs(base_logits - lora_logits).mean()
    print(f"[5] LoRA forward OK! Logits diff vs base: {diff:.6f}", flush=True)
except Exception as e:
    print(f"[5] Forward failed: {e}", flush=True)
    import traceback; traceback.print_exc()
    sys.exit(1)

# Manual decode
print("[6] Manual decode 20 tokens...", flush=True)
...
for i in range(20):
    ...
    if nt == 2:
        print(f"[6] EOS at token {i+1}", flush=True)
        break

result = processor.decode(gen_ids, skip_special_tokens=True)
print(f"[7] LoRA result: {result[:200]}", flush=True)
print("[8] SUCCESS! In-place addition works!", flush=True)
