"""
Evaluate all v4 training checkpoints + final model on easy50.
...
CKPT_DIR = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/checkpoints_v4"
FINAL_MODEL = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/lora_projector_v4_final_fp16.pdparams"
EVAL_DIR = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/lora_v4_eval"
...
# Checkpoints (evaluate all checkpoints in v4)
...
for name, ckpt_path in models_to_eval.items():
    ...
    output_path = str(DATASET_DIR / f"results_v4_{name}_easy50.jsonl")
...
# Summary
print("\n" + "="*60)
print("BEST CHECKPOINT SELECTION (V4)")
print("="*60)
...
with open(str(DATASET_DIR / "checkpoint_eval_results_v4.json"), "w") as f:
    json.dump({"results": results, "best": best_name}, f, indent=2)
print(f"\nResults saved to checkpoint_eval_results_v4.json")
