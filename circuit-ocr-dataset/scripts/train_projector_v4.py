"""
SAFE Projector-ONLY LoRA training v4 with checkpoints, diversity monitoring, and correct token alignment.
...
- Low learning rate: 5e-5 (to prevent catastrophic forgetting and mode collapse)
- Manual token concatenation: 100% alignment between prompt and label, preventing boundary discrepancy
"""
...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_v4"
...
log("=" * 60)
log("TRAINING V4 (RESOLUTION 384, MANUAL TOKEN ALIGNMENT, LR 5e-5)")
...
base_lr_decay = paddle.optimizer.lr.CosineAnnealingDecay(
    learning_rate=5e-5, T_max=total_steps - WARMUP_STEPS, eta_min=5e-6)
lr_scheduler = paddle.optimizer.lr.LinearWarmup(
    learning_rate=base_lr_decay, warmup_steps=WARMUP_STEPS, start_lr=5e-6, end_lr=5e-5)
    ...
    for idx, sample in enumerate(data):
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                best_ckpt_path = f"{OUTPUT_DIR}/lora_best_v4_fp16.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_projector_v4_final_fp16.pdparams"
...
log("Training complete! Evaluate checkpoints with eval_all_checkpoints_v3.py (configured for checkpoints_v4)")
