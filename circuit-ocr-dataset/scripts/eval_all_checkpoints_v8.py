"""
Evaluate all v8-fixed training checkpoints + final model on easy50.
Finds the best checkpoint by Avg. NED (using model.generate() directly).
"""
...
CKPT_DIR = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/checkpoints_v8_fixed"
FINAL_MODEL = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/lora_v8_fixed_final_fp16.pdparams"
EVAL_DIR = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/lora_v8_eval"
...
for name, ckpt_path in models_to_eval.items():
    ...
    output_path = str(DATASET_DIR / f"results_v8_{name}_limit10.jsonl")
    ...
    # Note: we use standard evaluation using model.generate() with a 15-minute timeout
    cmd = [
        PYTHON, EVAL_SCRIPT,
    ...
    try:
        ...
        # Calculate NED from output file
        if Path(output_path).exists():
            with open(output_path, encoding='utf-8') as f:
                data = [json.loads(l) for l in f if l.strip()]
            
            # Compute Lev. distance metrics
            import Levenshtein
            neds = []
            for d in data:
            ...
            if neds:
                ...
                print(f"  >> Avg. NED calculated: {ned:.4f}")
...
# Summary
print("\n" + "="*60)
print("BEST CHECKPOINT SELECTION (V8)")
print("="*60)
print("Results:")
...
with open(str(DATASET_DIR / "checkpoint_eval_results_v8_limit10.json"), "w") as f:
    json.dump({"results": results, "best": best_name}, f, indent=2)
print(f"\nResults saved to checkpoint_eval_results_v8_limit10.json")
