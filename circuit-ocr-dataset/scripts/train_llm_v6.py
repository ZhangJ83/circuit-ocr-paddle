"""
SAFE LLM-ONLY LoRA SFT training v6 with checkpoints and correct token alignment.
...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_v6"
...
log("=" * 60)
log("TRAINING V6 (LLM-ONLY LoRA r=16 alpha=32, RESOLUTION 384, LR 2e-5)")
    ...
    for idx, sample in enumerate(data):
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                best_ckpt_path = f"{OUTPUT_DIR}/lora_best_v6_fp16.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_projector_v6_final_fp16.pdparams"
