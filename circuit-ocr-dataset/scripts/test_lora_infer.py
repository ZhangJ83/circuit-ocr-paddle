"""Self-contained script: Load base model, merge LoRA in-place, test inference."""
...
def log(msg):
    """Print with flush and timestamp."""
    from datetime import datetime
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"[{ts}] {msg}", flush=True)

log("Starting Paddle patches...")

import paddle

# Dummy LocalSharedLayerDesc
...
log("Patches OK, setting device to GPU")

paddle.set_device("gpu")

# --- Now import PaddleFormers ---
...
log("Loading base model (float32)...")
...
for lora_base, adapters in lora_pairs.items():
    ...
    if wk in base_sd:
        ...
        if delta.shape == W.shape:
            ...
            merged += 1
model.eval()
log(f"Merged {merged}/{len(lora_pairs)} LoRA adapters into base model")
del lora_state
...
log(f"Loaded {len(samples)} samples, testing first 2...")
...
for idx in range(2):
    ...
    w, h = image.size
    max_dim = 768  # Same as eval_benchmark.py
    if max(w, h) > max_dim:
    ...
    log(f"[{idx+1}] {Path(img_path).name} ({image.size}) input_ids={list(inputs['input_ids'].shape)}")
    log(f"  Label: {label}")
    ...
    try:
        ...
        log(f"  Pred ({elapsed:.1f}s): {result[:200]}")
