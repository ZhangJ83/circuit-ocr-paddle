"""Test trained model with image input."""
...
PROCESSOR_PATH = r"F:\hf_cache\hub\models--PaddlePaddle--PaddleOCR-VL\snapshots\baee27eebcbf26cdeab160116679d765f13a3f27"
TRAINED_PATH = r"F:\hf_cache\hub\models--PaddlePaddle--PaddleOCR-VL\snapshots\baee27eebcbf26cdeab160116679d765f13a3f27"
DATA_DIR = r"G:\mimo_project\circuit_ocr\circuit-ocr-dataset"

log("[1] Loading processor...")
processor = AutoProcessor.from_pretrained(PROCESSOR_PATH)

log("[2] Loading trained model...")
...
log(f"  input_ids: {list(inputs['input_ids'].shape)}")
if "pixel_values" in inputs:
    log(f"  pixel_values: {inputs['pixel_values'].shape}")
if "image_grid_thw" in inputs:
    log(f"  image_grid_thw: {inputs['image_grid_thw']}")

# Manual decode
log("[4] Manual decode 30 tokens...")
...
for i in range(30):
    ...
    if nt == 2:
        log(f"  EOS at token {i+1}")
        break

elapsed = time.time() - t0
result = processor.decode(gen_ids, skip_special_tokens=True)
log(f"[5] Generated tokens: {gen_ids}")
result = processor.decode(gen_ids, skip_special_tokens=True)
log(f"[5] Result ({elapsed:.1f}s): {result[:200]}")
# Also decode without skip_special_tokens
result2 = processor.decode(gen_ids, skip_special_tokens=False)
log(f"[5] Raw result: {repr(result2[:200])}")
log("[6] SUCCESS!")
