"""One-shot: merge LoRA into base model, save as new checkpoint, test inference."""
...
MERGED_OUT = f"{LORA_DIR}/merged_base_f32"
LORA_SCALE = 2.0

# === Step 1: Merge LoRA into base model and save ===
print("=" * 60)
...
for lora_base, adapters in lora_pairs.items():
    ...
    if wk in base_sd:
        ...
        if delta.shape == W.shape:
            base_sd[wk].set_value(paddle.to_tensor(W + delta.astype(np.float32)))
            merged += 1
print(f"Merged {merged}/{len(lora_pairs)} LoRA adapters")

# Save as model checkpoint
os.makedirs(MERGED_OUT, exist_ok=True)
paddle.save(base_sd, f"{MERGED_OUT}/model_state.pdparams")
# Copy config files
import shutil
for fname in ["config.json", "generation_config.json", "preprocessor_config.json",
    ...
    if os.path.exists(src):
        shutil.copy2(src, f"{MERGED_OUT}/{fname}")

del model, lora_state, base_sd
paddle.device.cuda.empty_cache()
print(f"Merged model saved to {MERGED_OUT}")

# === Step 2: Reload merged model as regular PaddleOCR-VL ===
print("\n" + "=" * 60)
print("Step 2: Reload merged model and test inference")
print("=" * 60)

model2 = AutoModelForConditionalGeneration.from_pretrained(
    MERGED_OUT, convert_from_hf=True, load_checkpoint_format="naive",
    low_cpu_mem_usage=True, dtype="float32"
)
model2.config._attn_implementation = "flashmask"
model2.visual.config._attn_implementation = "flashmask"
model2.eval()
print("Merged model reloaded OK")

gen_config = GenerationConfig(do_sample=False, bos_token_id=1, eos_token_id=2, pad_token_id=0)
...
for idx in range(3):
    ...
    label = sample["messages"][1]["content"][:80].replace("\n", "|")
    print(f"\n[{idx+1}] {Path(img_path).name} ({image.size})", flush=True)
    print(f"  Label: {label}", flush=True)

    t0 = time.time()
    try:
        with paddle.no_grad():
            outputs = model2.generate(**inputs, generation_config=gen_config, max_new_tokens=200)
        elapsed = time.time() - t0
        input_len = inputs["input_ids"].shape[1]
        result = processor.decode(outputs[0][0][input_len:], skip_special_tokens=True)
        print(f"  Pred ({elapsed:.1f}s): {result[:150]}", flush=True)
