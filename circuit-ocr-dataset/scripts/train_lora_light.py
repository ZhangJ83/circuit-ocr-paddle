#!/usr/bin/env python3
"""PaddleOCR-VL LoRA Training — Light version for 8GB VRAM.
Filters to short-label samples, uses 64px images, truncated labels.
"""
...
# Logging
LOG_FILE = f"{DATASET_DIR}/PaddleOCR-VL-LoRA-circuit-ocr/training_light.log"
def log(msg):
...
def compute_loss(model, processor, sample):
    ...
    try:
        ...
        lt = processor.tokenizer(label, return_tensors="pd", padding=False, truncation=True, max_length=64)
...
def main():
    ...
    ap.add_argument('--max_label_len', type=int, default=100, help='Filter samples with label > this')
    ...
    all_train = [json.loads(l) for l in open(args.train_data) if l.strip()]
    # Filter: only samples with short labels
    train = [s for s in all_train if len(s["messages"][1]["content"]) <= args.max_label_len]
    log(f"Data: Filtered {len(train)}/{len(all_train)} samples (label <= {args.max_label_len} chars)")

    evals = [json.loads(l) for i, l in enumerate(open(args.eval_data)) if l.strip() and i < args.max_eval_samples]
    log(f"Eval: {len(evals)} samples")
    ...
    for ep in range(args.epochs):
        ...
        log(f"[Epoch {ep+1}] Done in {(time.time()-e0)/60:.1f}m (none_count={none_count})")

    tm = (time.time() - t0) / 60
    log(f"{'='*60}")
    log(f"Done! {tm:.0f}m ({tm/60:.1f}h) Best eval: {best:.4f} Steps: {gs}")
    log(f"{'='*60}")
    paddle.save(model.state_dict(), f"{args.output_dir}/final_model_light.pdparams")
    json.dump(hist, open(f"{args.output_dir}/loss_history_light.json", 'w'), indent=2)
    log("Complete.")
