"""Test: merge LoRA by modifying model parameters directly (not state_dict)."""
...
# === METHOD: Modify named_parameters directly ===
print("[4] Loading LoRA...", flush=True)
lora_state = paddle.load(f"{LORA_DIR}/lora_weights_f32.pdparams")

# Build param name → param tensor map
param_map = {}
for name, param in model.named_parameters():
    param_map[name] = param
print(f"[4] Model has {len(param_map)} named parameters", flush=True)

# Build LoRA pairs, map to param names
lora_pairs = {}
...
print(f"[4] {len(lora_pairs)} LoRA pairs", flush=True)

# Try to find matching parameter names
# The LoRA base names match state_dict keys like "visual.vision_model.encoder.layers.0.self_attn.q_proj"
# But named_parameters might use different naming
# Let's try both: named_parameters AND state_dict
sd = model.state_dict()
merged = 0
for lora_base, adapters in lora_pairs.items():
    ...
    # Try named_parameters first
    if lora_base in param_map:
        param = param_map[lora_base]
        W = param.numpy()
        if delta.shape == W.shape:
            param.set_value(paddle.to_tensor(W + delta.astype(np.float32)))
            merged += 1
            continue
    ...
    if wk in sd:
        W = sd[wk].numpy()
        if delta.shape == W.shape:
            sd[wk].set_value(paddle.to_tensor(W + delta.astype(np.float32)))
            merged += 1

model.eval()
print(f"[4] Merged {merged}/{len(lora_pairs)}", flush=True)

# Test forward after merge
print("[5] Testing LoRA forward...", flush=True)
paddle.device.cuda.empty_cache()
with paddle.no_grad():
    out2 = model(input_ids=inputs["input_ids"], pixel_values=inputs.get("pixel_values"),
                 image_grid_thw=inputs.get("image_grid_thw"), use_cache=False)
print(f"[5] LoRA forward OK, logits={list(out2[0].shape)}", flush=True)

# Manual decode 10 tokens
print("[6] Manual decode...", flush=True)
...
print(f"[7] Result: {result[:200]}", flush=True)
