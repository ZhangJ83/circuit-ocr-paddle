#!/usr/bin/env python3
"""
WSL CPU-mode PaddleOCR-VL Benchmark Script
==========================================
Runs PaddleOCR-VL-0.9B in WSL CPU mode to bypass Windows Paddle 2.6.2
C++ stack overflow bug (MSVC 1MB thread stack vs Linux 8MB default).

Uses JPEG pre-converted 384x384 images for stable processing.
"""
import os, sys

# Use WSL Linux paths (mounted drives)
...
def apply_paddle_patches():
    ...
    try:
        ...
        # Patch creation functions to ignore 'device'
        ...
        # Patch PaddleFormers repeat_kv .expand bug (PyTorch-style API vs Paddle)
        ...
        def patched_repeat_kv(hidden_states, n_rep):
            ...
            # Paddle .expand takes a single shape argument, not multiple positional args
            hidden_states = hidden_states[:, :, None, :, :].expand([batch, num_key_value_heads, n_rep, slen, head_dim])
            return hidden_states.reshape([batch, num_key_value_heads * n_rep, slen, head_dim])
        ea.repeat_kv = patched_repeat_kv

        # Global patch: paddle.Tensor.expand to accept PyTorch-style positional args
        old_expand = paddle.Tensor.expand
        def patched_expand(self, *args, **kwargs):
            if len(args) > 1:
                # PyTorch-style: expand(a, b, c, ...)
                shape = list(args)
...
def parse_args():
    parser = argparse.ArgumentParser(description="WSL CPU PaddleOCR-VL Benchmark")
...
def evaluate_paddleocr_vl(args):
    ...
    # Force CPU mode - Paddle 2.6.2 GPU crashes with cuDNN in WSL
    device = "cpu"
    print(f"Setting Paddle device to: {device}")
    paddle.set_device(device)

    # Use WSL Linux path to local HF cache
    ...
    generation_config = GenerationConfig(
        ...
        eos_token_id=2,
        pad_token_id=0,
        use_cache=True  # Safe on Linux 8MB stack, now that expand() is patched
    )
...
def main():
    ...
    print("=" * 40)
    print(f"Model Type: paddleocr-vl (WSL CPU)")
