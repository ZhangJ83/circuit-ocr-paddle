"""Projector-Only LoRA r=16 — Colab compatible (no hardcoded paths).
Usage: python train_projector_only_colab.py
"""
...
# ── Paths (Colab-compatible) ──
MODEL_PATH = os.environ.get("MODEL_PATH", "/content/models/PaddleOCR-VL")
...
# ── Config ──
MAX_DIM = int(os.environ.get("MAX_DIM", "336"))
EPOCHS = int(os.environ.get("EPOCHS", "3"))
LR = float(os.environ.get("LR", "5e-4"))

# Projector ONLY — no LLM attention LoRA
TARGETS = [
    ".*linear_1", ".*linear_2",    # Projector (mlp_AR)
]

log(f"Config: max_dim={MAX_DIM}, epochs={EPOCHS}, lr={LR}")
...
# ── Data ──
train_file = f"{DATA_DIR}/ocr_vl_sft-train.jsonl"
if not os.path.exists(train_file):
    log(f"Downloading training data...")
    import urllib.request
    url = "https://github.com/ZhangJ83/circuit_ocr_dataset_final/raw/main/ocr_vl_sft-train.jsonl"
...
lr_scheduler = paddle.optimizer.lr.CosineAnnealingDecay(
    learning_rate=LR, T_max=total_steps, eta_min=LR/10)
...
for epoch in range(EPOCHS):
    ...
    for idx, sample in enumerate(data):
        ...
        img_path = f"{DATA_DIR}/{img_rel}"
        if not os.path.exists(img_path):
            continue  # Skip missing images in Colab
...
if os.path.exists(test_path):
    ...
    for i, s in enumerate(test_data):
        ...
        try:
            ...
            tok = out[0] if isinstance(out, (list,tuple)) else out
        ...
        log(f"  [{i}] Pred({len(resp)}ch): {resp[:100]}")
