...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_exp6"
...
model = AutoModelForConditionalGeneration.from_pretrained(
    MODEL_PATH, load_checkpoint_format="safetensors",
    ...
    for idx, sample in enumerate(train_data):
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(train_data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                # Save as latest best
                best_path = f"{OUTPUT_DIR}/lora_exp6_best.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_exp6_final.pdparams"
