"""
SAFE LLM-ONLY LoRA SFT training v7 with checkpoints and CORRECT next-token label shifting.
- Fixes label shifting: labels_t[0, prompt_len - 1 : prompt_len + label_len - 1] = label_ids
  Ensures the model learns next-token prediction instead of identity copying.
- Targets ONLY the LLM decoder self-attention layers: model.layers.X.self_attn.{q,k,v,o}_proj
- LoRA parameters: r=16, alpha=32 (user request)
- Freezes visual encoder and projector (mlp_AR) to preserve pre-trained VL alignment and prevent collapse
- Improved dataset: V5-Golden (2,299 clean samples, balanced and format-aligned)
- max_dim=384 (keep text readable and fast)
...
- Robust manual greedy decode in quick_inference (bypasses buggy LoRAModel.generate() wrapper)
"""
...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_v7"
...
log("=" * 60)
log("TRAINING V7 (LLM-ONLY LoRA r=16 alpha=32, RESOLUTION 384, LR 2e-5)")
...
# ── Quick inference helper (Manual Greedy Decoder, bypasses buggy generate()) ──
def quick_inference(samples, max_tokens=60):
    ...
    for s in samples:
        try:
            ...
            # Manual greedy decoding step-by-step
...
for epoch in range(EPOCHS):
    ...
    for idx, sample in enumerate(data):
        ...
        # Labels for cross entropy calculation (shifted by 1 for next-token prediction)
        labels_t = paddle.full([1, prompt_len + label_len], fill_value=-100, dtype="int64")
        labels_t[0, prompt_len - 1 : prompt_len + label_len - 1] = label_ids
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                best_ckpt_path = f"{OUTPUT_DIR}/lora_best_v7_fp16.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_projector_v7_final_fp16.pdparams"
