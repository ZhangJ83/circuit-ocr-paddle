...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_v10_clean"
...
log(f"  Dataset: output/train_v10fmt.jsonl (v4 synth 2000 + real KiCad 1839, NO Masala)")
...
# ── Data ──
with open(f"{DATASET_DIR}/output/train_v10fmt.jsonl", encoding="utf-8") as f:
...
for epoch in range(EPOCHS):
    ...
    for idx, sample in enumerate(train_data):
        ...
        image.close()
            ...
            log(f"  SKIP {idx}: {str(e)[:50]}")
            opt.clear_grad()
            image.close() if 'image' in dir() else None
            continue
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(train_data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                # Save as latest best
                best_path = f"{OUTPUT_DIR}/lora_best_v10_clean.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_v10_clean_final.pdparams"
