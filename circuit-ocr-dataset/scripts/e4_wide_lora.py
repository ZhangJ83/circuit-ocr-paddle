"""E4: Broader LoRA coverage — add Projector + vision encoder layers.
Hypothesis: freezing the vision→language Projector causes mode collapse.
"""
...
# E4 config: same size/duration as E1 baseline for fair comparison
MAX_DIM = 168
NUM_SAMPLES = 200
EPOCHS = 1
...
# ── Config A: original (q/k/v/o only) ── CONFIRMS the bug
# ── Config B: + Projector + vision self_attn + FFN  ── THE FIX

TARGETS_CONFIG = {
    "A_base": [".*q_proj", ".*k_proj", ".*v_proj", ".*o_proj"],  # original
    "B_projector": [                                            # + Projector only
        ".*q_proj", ".*k_proj", ".*v_proj", ".*o_proj",         # self-attn
        ".*mlp_AR.linear_1", ".*mlp_AR.linear_2",               # Projector (vision→language)
    ],
}

for config_name, target_modules in TARGETS_CONFIG.items():
    log(f"=== {config_name}: targets={target_modules} ===")
    ...
    lc = LoRAConfig(r=8, lora_alpha=16, target_modules=target_modules)
    ...
    for epoch in range(EPOCHS):
        ...
        for idx, sample in enumerate(data):
            ...
            if global_step % 50 == 0:
                ...
                log(f"  [S{global_step}/{total_steps}] loss={loss.item():.4f} elapsed={elapsed:.0f}m")

    elapsed = (time.time()-t0)/60
    log(f"  Training done in {elapsed:.1f}m")
    ...
    del model; paddle.device.cuda.empty_cache()
    log("")
