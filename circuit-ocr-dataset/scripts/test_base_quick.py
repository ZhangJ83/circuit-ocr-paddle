"""Quick test: base model inference without LoRA, 256px images, 2 samples."""
...
print("Loading base model (float32)...", flush=True)
...
model.eval()
print("Base model loaded OK", flush=True)
...
for idx in range(2):
    ...
    print(f"  input_ids: {inputs['input_ids'].shape}", flush=True)
    if "pixel_values" in inputs:
        print(f"  pixel_values: {inputs['pixel_values'].shape}, dtype={inputs['pixel_values'].dtype}", flush=True)
    if "image_grid_thw" in inputs:
        print(f"  image_grid_thw: {inputs['image_grid_thw']}", flush=True)

    t0 = time.time()
    try:
        with paddle.no_grad():
            outputs = model.generate(**inputs, generation_config=gen_config, max_new_tokens=50)
