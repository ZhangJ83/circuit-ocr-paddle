...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_exp5"
...
log(f"  Dataset: output/train_v10fmt_synth.jsonl (1200 clean + 300 synth text, ratio 20%)")
...
lc = LoRAConfig(r=16, lora_alpha=32, target_modules=TARGETS, lora_dropout=0.10)
...
# ── Data ──
with open(f"{DATASET_DIR}/output/train_v10fmt_synth.jsonl", encoding="utf-8") as f:
    ...
    for idx, sample in enumerate(train_data):
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(train_data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                # Save as latest best
                best_path = f"{OUTPUT_DIR}/lora_exp5_best.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_exp5_final.pdparams"
