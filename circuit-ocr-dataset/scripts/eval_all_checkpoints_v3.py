"""
Evaluate all v3 training checkpoints + final model on easy50.
...
CKPT_DIR = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/checkpoints_v3"
FINAL_MODEL = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/lora_projector_v3_final_fp16.pdparams"
EVAL_DIR = DATASET_DIR / "PaddleOCR-VL-LoRA-circuit-ocr/lora_v3_eval"
...
for name, ckpt_path in models_to_eval.items():
    ...
    output_path = str(DATASET_DIR / f"results_v3_{name}_easy50.jsonl")
    ...
    # Note: we do NOT use --manual_decode here as Paddle 3.1.0 is stable.
    cmd = [
        PYTHON, EVAL_SCRIPT,
        ...
        "--max_length", "128", # increased generation max_length for full labels
    ]
...
# Summary
print("\n" + "="*60)
print("BEST CHECKPOINT SELECTION (V3)")
print("="*60)
...
# Save results
with open(str(DATASET_DIR / "checkpoint_eval_results_v3.json"), "w") as f:
    json.dump({"results": results, "best": best[0] if valid else None}, f, indent=2)
print(f"\nResults saved to checkpoint_eval_results_v3.json")
