"""Projector-ONLY LoRA: zero LLM attention LoRA, only visual-language bridge.
r=32, alpha=64, targets: mlp_AR.linear_1 + linear_2 only.
Tests whether LLM LoRA is the root cause of shortcut collapse. 3 epochs, 2433 samples.
"""
...
# KEY: remove k/v to force model to use visual features
# GQA has only 2 KV heads — LoRA on k/v lets LLM ignore weak visual signal
TARGETS = [
    ".*linear_1", ".*linear_2",    # Projector ONLY — NO LLM attention LoRA
]

log(f"Projector-ONLY LoRA r=32: targets={TARGETS}")
...
lc = LoRAConfig(r=32, lora_alpha=32, target_modules=TARGETS)
...
save_path = f"{OUTPUT_DIR}/lora_projector_only_r32_fp16.pdparams"
