...
# === Test with 64px images (same as training that works) ===
...
for idx in range(5):
    ...
    w, h = image.size
    # Resize to 64px max (same as training's 32px — safe margin)
    max_dim = 64
    if max(w, h) > max_dim:
    ...
    label_short = sample["messages"][1]["content"][:80].replace('\n','\\n')
    print(f"  Label: {label_short}", flush=True)
    ...
    try:
        ...
        pred_short = result[:120].replace('\n','\\n')
        print(f"  Pred ({elapsed:.1f}s): '{pred_short}'", flush=True)
    except Exception as e:
        print(f"  ERROR ({time.time()-t0:.1f}s): {e}", flush=True)
    image.close()
    paddle.device.cuda.empty_cache()

print("\nAll 5 images done!", flush=True)
