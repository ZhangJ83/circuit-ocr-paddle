"""Step 2 v2: Projector LoRA r=32 with stabilization fixes.
- lr: 2e-4 (vs 5e-4) to prevent gradient spikes
- Warmup: 500 steps linear warmup
- Gradient clipping: max_norm=1.0
r=32, alpha=64. 3 epochs, all 2433 samples.
"""
...
log(f"Projector LoRA r=32 v2 (lr=1e-4, clip=1.0): targets={TARGETS}")
...
lc = LoRAConfig(r=32, lora_alpha=64, target_modules=TARGETS)
...
lr_scheduler = paddle.optimizer.lr.CosineAnnealingDecay(
    learning_rate=1e-4, T_max=total_steps, eta_min=1e-5)
opt = paddle.optimizer.AdamW(
    learning_rate=lr_scheduler, parameters=[p for p in model.parameters() if not p.stop_gradient],
    weight_decay=0.1, grad_clip=paddle.nn.ClipGradByNorm(clip_norm=1.0))
...
save_path = f"{OUTPUT_DIR}/lora_projector_r32_v2_fp16.pdparams"
