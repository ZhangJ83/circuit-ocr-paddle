"""Test: merge LoRA via set_state_dict (replace all params at once)."""
...
# Build deltas map: state_dict_key -> numpy delta
deltas = {}
...
# Build new state_dict — keep tensors on same device as original
new_sd = {}
merged = 0
for k, v in model.state_dict().items():
    if k in deltas:
        ...
        if delta.shape == W.shape:
            new_sd[k] = paddle.to_tensor(W + delta.astype(np.float32), place=v.place)
            merged += 1
        else:
            new_sd[k] = v
    else:
...
print(f"[4] Built new state_dict, {merged}/{len(deltas)} keys merged", flush=True)
print(f"[4] Calling model.set_state_dict()...", flush=True)
model.set_state_dict(new_sd)
model.eval()
print(f"[4] set_state_dict OK", flush=True)

# Test forward after set_state_dict
...
print(f"[5] Logits diff (base vs lora): {diff:.6f}", flush=True)
