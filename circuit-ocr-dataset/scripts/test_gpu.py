#!/usr/bin/env python3
"""Test PaddleOCR-VL GPU inference in WSL with conda CUDA 11.8."""
import os, sys, time
...
model = AutoModelForConditionalGeneration.from_pretrained(
    ...
    low_cpu_mem_usage=True, dtype="float16"
)
...
t0 = time.time()
with paddle.no_grad():
    out = model.generate(**inp, generation_config=gc, max_new_tokens=200)
    elapsed = time.time() - t0
    oids = out[0].tolist()[0]
    txt = proc.decode(oids, skip_special_tokens=True)
    print(f"GPU inference: {elapsed:.1f}s, tokens={len(oids)}, text=[{txt}]")
print("GPU MODEL TEST PASSED!")
