"""Minimal LoRA inference test — 64px images, bf16, use_cache=True (WSL-proven pattern)."""
...
os.environ["PATH"] = r"E:\080000software\080900_Miniconda\miniconda3\Library\bin;" + \
...
# Patch safetensors PySafeSlice (critical for bf16 loading on Paddle 2.6.2)
try:
    import numpy as np
    import tempfile
    ...
    if os.path.exists(tmp_path):
        os.remove(tmp_path)
    print("[PySafeSlice] Patched", flush=True)
except Exception as e:
    print(f"[PySafeSlice] Patch failed: {e}", flush=True)
...
for lora_base, adapters in lora_pairs.items():
    ...
    if wk in base_sd:
        ...
        if delta.shape == W.shape:
            ...
            merged += 1
model.eval()
print(f"Merged {merged}/{len(lora_pairs)} LoRA adapters", flush=True)

# Test with small images (WSL-proven 64px pattern)
...
for idx in range(3):
    ...
    w, h = image.size
    max_dim = 64  # WSL-proven safe size
    if max(w, h) > max_dim:
    ...
    try:
        with paddle.no_grad():
            outputs = model.generate(**inputs, generation_config=gen_config, max_new_tokens=200)
