"""
SAFE Projector-ONLY LoRA training v3 with checkpoints and diversity monitoring.
- Improved dataset: 1,857 samples (1,357 real + 500 new synthetic)
- Projector-only targets (.*linear_1, .*linear_2)
- max_dim=384 (increased from 168 to keep text readable)
- Gradient accumulation (4 steps)
- Gradient clipping (1.0)
- Learning rate warmup (100 steps)
- Full labels (no truncation to 200 chars)
- Checkpoint every 200 steps with 3-sample inference test to monitor diversity
"""
...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_v3"
...
# ── Config ──
MAX_DIM = 384          # Resolution increased from 168 to 384
EPOCHS = 2             # Reduced from 3 to prevent overfit
GRAD_ACCUM = 4         # Gradient accumulation to stabilize training
GRAD_CLIP = 1.0        # Gradient clipping norm
CHECKPOINT_STEPS = 200  # Checkpoint interval (in optimizer steps)
TARGETS = [
    ".*linear_1", ".*linear_2",    # Projector ONLY
]

log("=" * 60)
log("TRAINING V3 (RESOLUTION 384, NO TRUNCATION)")
...
base_lr_decay = paddle.optimizer.lr.CosineAnnealingDecay(
    learning_rate=2e-4, T_max=total_steps - WARMUP_STEPS, eta_min=2e-5)
lr_scheduler = paddle.optimizer.lr.LinearWarmup(
    learning_rate=base_lr_decay, warmup_steps=WARMUP_STEPS, start_lr=2e-5, end_lr=2e-4)
...
monitor_samples = test_data[:3]  # Use first 3 samples for verification
...
for epoch in range(EPOCHS):
    ...
    for idx, sample in enumerate(data):
        ...
        label = sample["messages"][1]["content"]  # Full label, NO truncation
        ...
        if (idx + 1) % GRAD_ACCUM == 0 or idx == len(data) - 1:
            ...
            if global_step % CHECKPOINT_STEPS == 0:
                ...
                # Check for diversity/collapse (optional warning)
                unique_preds = len(set(preds))
                log(f"    Monitor Diversity: {unique_preds}/{len(preds)}")
                
                best_ckpt_path = f"{OUTPUT_DIR}/lora_best_v3_fp16.pdparams"
...
model.eval()
lora_dict = {k: paddle.cast(p.detach(), "float16") for k, p in model.named_parameters() if 'lora_' in k}
final_path = f"{OUTPUT_DIR}/lora_projector_v3_final_fp16.pdparams"
