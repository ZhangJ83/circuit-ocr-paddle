...
本教程旨在提供基于 PaddleFormers 微调 PaddleOCR-VL 模型适配孟加拉语的微调教程，资源需求 and 运行耗时见下方表格：
...
- **messages**：文本数据列表，记录了用户与模型之间的交互过程，其中每个元素包含一个 `role` 和一个 `content`。
  ...
  - **content**：消息的具体内容。
    - 输入端包含指令和图片占位符。
      - **提示指令 Prompt**：根据识别任务设置，例如文字识别 `"OCR:"`（最通用）、表格识别 `"Table Recognition"`、公式识别 `"Formula Recognition"`、图表识别 `"Chart Recognition"`，或者根据微调任务自定义提示。
      - **图片占位符 `<image>`**：在文本数据中标记图片插入的位置。
    - 输出端包含模型预期生成的正确答案，即图片中需要识别的字符。
- **images**：图像数据列表，存储了对话中涉及到的图片路径（本地路径或 URL）。

#### 自行准备数据
如果您想要基于自己的数据集进行训练，请参考数据集格式说明准备数据。

#### 其他任务格式
表格/公式/图表数据会使用特殊的识别格式，细节请参考后文的“表格/公式/图表数据格式”部分。
...
- `gradient_accumulation_steps=8`：梯度累积步数。每达到该步数整数倍更新一次模型参数。当显存不足时，可以减小 `per_device_train_batch_size` 并增大 `gradient_accumulation_steps`。用时间换空间策略，可以减少显存占用，但会延长训练时间。
...
# Expected Answer = নট চলল রফযনর পঠ সওযর\nহয গলয গলয ভব এখন দটত, মঝ মঝ খবর নয যদও লগ যয\nঝগড\nদরগর কছ চল এল
```
...
### 6.3 部署推理
部署 PaddleOCR-VL 模型，请参考《基于 FastDeploy / vLLM 部署模型》和《FastDeploy - PaddleOCR-VL-0.9B Best Practices》
...
### 7.1 昆仑芯 P800
XPU 环境配置请参考 XPU 安装说明文档。使用以下命令行即可启动全参训练：
```bash
...
| SFT | 显存 | 用时 | 指标 (Avg. NED) |
| --- | --- | --- | --- |
| 全参 | 30G | 3h 22min | 0.0067 |
| LoRA | 20G | 3h 33min | 0.0065 |

### 7.2 天数智芯 150s
ILUVATAR-GPU 环境配置请参考 ILUVATAR-GPU 安装说明文档。使用以下命令行即可启动全参训练：
```bash
...
```
更多的 OTSL 格式控制符以及具体意义如下所示：
- `<ecel>`: 结束当前单元格（End Cell）。用于标记单元格的结束。
- `<fcel>`: 开始一个新的单元格（First Cell）。通常用于表格中的第一个单元格。
- `<xcel>`: 开始一个新的单元格（eXtended Cell）。用于表格中除第一个单元格外的其他单元格。
- `<lcel>`: 结束当前行并开始新行（Last Cell）。用于标记一行的结束。
- `<ucel>`: 合并单元格（Union Cell）。用于表示跨多行或多列的合并单元格。
- `<nl>`: 换行（New Line）。用于文本中的换行操作。

表格建议使用 PPOCRLabel 标注，标注结果为 HTML 格式表格，可以使用脚本将 HTML 格式转化为 OTSL 格式。
模型识别输出的 OTSL 格式表格，可以参考 `convert_otsl_to_html` 将 OTSL 转 HTML 来观察识别得到的表格是否符合预期。
