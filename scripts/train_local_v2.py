"""Local training on RTX 4060 8GB — manual input construction (proven working)."""
import os, sys, json, time, random, numpy as np

# flex_checkpoint dummy
...
OUTPUT_DIR = os.path.join(DATASET_DIR, "checkpoints", "local_v2")
...
log(f"LOCAL-V2: max_dim={MAX_DIM} epochs={EPOCHS} lr={BASE_LR:.0e} grad_accum={GRAD_ACCUM}")
...
# Data
with open(os.path.join(DATASET_DIR, "output", "train_clean.jsonl"), encoding="utf-8") as f:
...
for epoch in range(EPOCHS):
    ...
    for idx, s in enumerate(train_data):
        try:
            ...
            img_np  # use img (PIL) for processor, img_np kept for other uses = np.array(img)
            ...
            img_inputs = processor.image_processor(images=[img_np  # use img (PIL) for processor, img_np kept for other uses], return_tensors="np")
            igt = img_inputs["image_grid_thw"][0]
            n_copies = max(1, int(igt[1]) * int(igt[2]) // 4)
            prompt = ('<' + '|placeholder|' + '>') * n_copies + 'OCR:'
            inp = processor(text=[prompt], images=[img_np  # use img (PIL) for processor, img_np kept for other uses], return_tensors="np", padding=True, max_length=2048, truncation=True)
            ...
            if (idx + 1) % GRAD_ACCUM == 0 or idx == len(train_data) - 1:
                ...
                if gs % CHECKPOINT_STEPS == 0:
                    ...
                    preds = []; refs = []
                    with paddle.no_grad():
                        for vi, vs in enumerate(val_fixed):
                            try:
                                vip = vs['images'][0]
                                if not os.path.exists(vip): vip = vip.replace("/root/circuit_ocr/", DATASET_DIR + "/")
                                ...
                                vimg_np  # use img (PIL) for processor, img_np kept for other uses = np.array(vimg)
                                vfeats = processor.image_processor(images=[vimg_np  # use img (PIL) for processor, img_np kept for other uses], return_tensors="np")
                                vig = vfeats["image_grid_thw"][0]
                                vn = max(1, int(vig[1])*int(vig[2])//4)
                                vprompt = ('<' + '|placeholder|' + '>')*vn + 'OCR:'
                                vinp = processor(text=[vprompt], images=[vimg_np  # use img (PIL) for processor, img_np kept for other uses], return_tensors="np", padding=False, max_length=2048, truncation=True)
                                vipd = {}
                                for k, v in vinp.items():
                                    if isinstance(v, np.ndarray): vipd[k] = paddle.to_tensor(v)
                                    elif hasattr(v, 'numpy'): vipd[k] = paddle.to_tensor(v.numpy())
                                    else: vipd[k] = v
                                vipd['pixel_values'] = paddle.to_tensor(vfeats['pixel_values']) if isinstance(vfeats['pixel_values'], np.ndarray) else paddle.to_tensor(vfeats['pixel_values'].numpy())
                                vipd['image_grid_thw'] = paddle.to_tensor(vfeats['image_grid_thw']) if isinstance(vfeats['image_grid_thw'], np.ndarray) else paddle.to_tensor(vfeats['image_grid_thw'].numpy())
                                # Use model.model.generate() to bypass LoRA wrapper
                                vout = model.model.generate(**vipd, generation_config=gc, max_new_tokens=256)
                                preds.append(processor.tokenizer.decode(vout[0].tolist()[0], skip_special_tokens=True))
                                refs.append(vs['messages'][1]['content']); vimg.close()
                            except Exception as e:
                                preds.append("[ERR]"); refs.append(vs['messages'][1]['content'])
                                if vi == 0: log(f"  VAL_ERR: {str(e)[:80]}")
                    model.train()
                    if preds and preds[0] != "[ERR]":
                        m = compute_all(preds, refs, label=f"s{gs}")
                        log(f"  Val: jf1={m['joint_f1']:.4f} CompF1={m['component_f1']:.4f}")
                        log(f"  Pred[0]: {preds[0][:80]}")
                    else:
                        log(f"  Val: all ERR - {preds[0] if preds else 'EMPTY'}")
        except Exception as e:
