...
# Patch: PySafeSlice.shape for safetensors compat (Paddle 3.1.0)
try:
    import numpy as np, tempfile
    from safetensors.numpy import save_file as _sf, safe_open as _so
    _tmp = tempfile.mktemp(suffix='.safetensors')
    _sf({'dummy': np.zeros((1,))}, _tmp)
    with _so(_tmp, framework='np') as _f:
        _PySafeSlice = type(_f.get_slice('dummy'))
        setattr(_PySafeSlice, 'shape', property(lambda self: self.get_shape()))
    if os.path.exists(_tmp): os.remove(_tmp)
except Exception as _e:
    print(f"Warning: PySafeSlice patch failed: {_e}", file=sys.stderr)
...
# Paddle 3.1.0 compat: LongTensor was removed
...
CKPT_DIR = f"{OUTPUT_DIR}/checkpoints_v11"
...
log("=" * 60)
log("TRAINING V11 (Phase 2: v4 synth + real KiCad + regularization)")
...
# ── Final Report ──
log("=" * 60)
log("TRAINING V11 (Phase 2) SUMMARY")
