  ...
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \begin{block}{本项目工作}
        \footnotesize
        \begin{enumerate}
          \item 构建首个电路 OCR 高质量标注数据集（7 轮验证）
          \item 提出合成文字数据防塌缩策略
...
\begin{frame}{标注质量保证}
  \begin{columns}[T]
    ...
    \column{0.55\textwidth}
      ...
      \end{block}
      \begin{center}
        \footnotesize 质量报告含 12 组 GT-vs-Image 可视化对比
      \end{center}
  \end{columns}
\end{frame}
...
\begin{frame}{数据集关键指标一览}
  \begin{center}
    \includegraphics[width=0.85\textwidth]{../slides_figures/dataset_metrics_infographic.pdf}
  \end{center}
\end{frame}

\begin{frame}{元件类型分布}
  \begin{center}
    \includegraphics[width=0.85\textwidth]{../slides_figures/component_types.pdf}
  \end{center}
  \begin{center}
...
\begin{frame}{视觉复杂度：远超通用文档}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{../slides_figures/visual_complexity_real.pdf}
  \end{center}
  \begin{center}
...
\begin{frame}{结构复杂度：五个隐式子任务}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{../slides_figures/multitask_complexity.pdf}
  \end{center}
  \begin{block}{}
    \footnotesize
...
\begin{frame}{Phase 2：合成数据防塌缩（1,500 样本）}
  ...
  \vspace{0.3cm}
  \begin{center}
    \footnotesize \textbf{合成数据是当前最有效的防塌缩手段：CompF1 提升 4.2 倍}
  \end{center}
\end{frame}
...
\begin{frame}{训练配置与消融总结}
  \begin{table}
    ...
    \begin{tabular}{lcccl}
      ...
      合成数据 & 无 vs 有 & 0.028 vs 0.126 & \textbf{关键突破} \\
  ...
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \begin{block}{下一步}
        \footnotesize
        \begin{enumerate}
          \item 扩大训练数据至 3,000+ 样本
          \item 两阶段训练：合成预训练到真实微调
          \item 更大 LoRA 秩 + 更多数据配合
          \item RL + SPICE 网表格式对齐
        \end{enumerate}
      \end{block}
  \end{columns}
\end{frame}
