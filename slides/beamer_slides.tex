...
\begin{frame}{基座模型 vs 微调模型}
  \begin{table}
    ...
    \begin{tabular}{lccc}
      ...
      exp6（v1, 合成文字混合）& 0.156 & 0.022 & 0.946 \\
      \textbf{Phase 1（v2, KiCad预训练）} & \textbf{0.304} & \textbf{0.041} & \textbf{0.942} \\
      \bottomrule
    \end{tabular}
  \end{table}
  \begin{block}{}
    ...
    LoRA 微调后 CompF1 从零提升至 0.156，LineAcc 为 0.022——输出开始包含电路领域词汇。
  \end{block}
\end{frame}
...
\begin{frame}{合成预训练实验结果}
  ...
  \begin{block}{关键发现}
    \footnotesize
    v2 在 CompF1（2.5$\times$）、LineAcc（1.9$\times$）、NED 和塌缩率上全面超越 v1，为本项目最佳模型。
  \end{block}
\end{frame}
    ...
    \column{0.5\textwidth}
      \begin{block}{数据瓶颈}
        ...
        \begin{itemize}
          ...
          \item CompF1=0.304 但 LineAcc 仅 0.041，行级匹配率有待提升
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      \begin{block}{当前局限}
        \footnotesize
        \begin{itemize}
          \item LineAcc = 0.041，v2 较 v1 提升 1.9$\times$ 但仍需提高
          \item 最优 CompF1 = 0.304，但 ExactMatch = 0\%
          \item 仅 1,200 真实训练样本（单 CAD 来源）
          \item 合成$\to$真实两阶段微调塌缩（性能下降）
