...
\begin{frame}{合成预训练实验结果}
  \begin{table}
    ...
    \begin{tabular}{lcccc}
      ...
      exp6（合成文字20\%混合）& 0.156 & 0.011 & 0.941 \\
      \textbf{Phase 1 合成预训练} & \textbf{0.304} & 0.005 & 0.942 \\
      Phase 1 + 真实数据微调 & 塌缩 & 0.000 & — \\
      \bottomrule
    \end{tabular}
  \end{table}
  \vspace{0.2cm}
  \begin{center}
    \footnotesize \textbf{CompF1 提升 2.5 倍（0.156 $\to$ 0.304），但 JointF1 未见改善}
  \end{center}
  \begin{block}{}
    \footnotesize
    纯合成KiCad预训练大幅改善元件标号识别，但标号-参数值配对仍是瓶颈。
    阶段2真实数据微调塌缩，需更精细的领域适配策略。
  \end{block}
\end{frame}
