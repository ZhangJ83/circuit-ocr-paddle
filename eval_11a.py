...
def eval_ckpt(path,name):
    ...
    model.eval()
    print(f'{name}: {n} params loaded. Eval...',end=' ',flush=True)
    preds=[]
...
print('=== EXP11A vs EXP6 ===')
cf11,jf11=eval_ckpt(os.path.join(D,'checkpoints','exp11a','best.pdparams'),'exp11a')
cf6,jf6=eval_ckpt(os.path.join(D,'PaddleOCR-VL-LoRA-circuit-ocr','lora_exp6_best.pdparams'),'exp6')
print(f'exp11a: CompF1={cf11:.4f} JointF1={jf11:.4f}')
print(f'exp6:   CompF1={cf6:.4f} JointF1={jf6:.4f}')
if jf11>jf6:print(f'exp11a JointF1 improved: {jf6:.4f} -> {jf11:.4f}')
else:print(f'exp6 still best JointF1')
if cf11>cf6:print(f'exp11a CompF1 improved: {cf6:.4f} -> {cf11:.4f}')
