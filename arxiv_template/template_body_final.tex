...
\begin{abstract}
我们提出 CircuitOCR——一个基于 PaddleOCR-VL-0.9B 的电路原理图 OCR 系统，涵盖数据集构建、多阶段受控实验与完整开源生态。主要贡献如下：
...
(3) \textbf{系统消融实验}：Phase 1--4 共 9 组受控实验，系统对比分辨率、学习率、dropout、投影器状态、合成数据混合、LoRA 秩、SPICE 格式微调及强化学习奖励引导微调。关键发现：冻结 Projector 层可防止模态塌缩；数据质量优先于数据数量。
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{系统消融}：Phase 1--4 共 9 组实验（exp1--exp9），覆盖分辨率/LoRA秩/投影器/SPICE/RL 等维度；
...
\begin{enumerate}
    ...
    \item \textbf{系统消融}：9 组实验系统对比分辨率、LoRA秩、投影器、合成数据、SPICE、RL 等维度。关键发现：冻结 Projector 防止塌缩；数据质量优先于数据数量；
