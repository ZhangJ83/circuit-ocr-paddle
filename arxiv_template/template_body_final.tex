...
\textbf{模型}：通过 Phase 1（四组控制变量实验：基线/高分辨率/防过拟合/解冻投影器）和 Phase 2（合成文字数据混合策略）两个阶段的实验，发现合成文字数据以 20\% 比例混入训练集可有效对抗小数据集上的模态塌缩，CompF1 提升 4.2$\times$（0.037 $\to$ 0.156），JointF1 提升 2.2$\times$（0.005 $\to$ 0.011）。进一步通过 kicad-cli 程序化生成 5,000 张 KiCad 原理图进行纯合成预训练（两阶段：合成预训练 $\to$ 真实数据微调），CompF1 达 0.304（较 exp6 提升约 2.5 倍），NED = 0.942，但 JointF1 仅 0.005——标号-参数值配对仍是核心瓶颈。所有训练在单卡 RTX 4060 8GB 上完成。模型权重、数据集和在线 Demo 均已完全开源。
\end{abstract}
...
\begin{enumerate}[nosep]
    \item \textbf{标注数据集}：经 7 轮验证的 Dataset A（1,820 样本），含标注质量报告与多维难度标签；
    ...
    \item \textbf{大规模合成KiCad预训练}：kicad-cli 程序化生成 5,000 张原理图，纯合成预训练使 CompF1 达 0.304，验证合成数据规模化路径；
...
\begin{enumerate}
    \item \textbf{电路原理图 OCR 数据集}：Dataset A 含 1,820 个经 7 轮验证的标注样本，覆盖 10 种元件类型，提供标注质量报告和多维难度标签；
    \item \textbf{合成数据防塌缩与规模化策略}：300 张合成文字图片以 20\% 比例混入训练集（CompF1 0.156），进一步通过 kicad-cli 程序化生成 5,000 张合成原理图进行纯合成预训练，CompF1 提升至 0.304（较 exp6 提升约 2.5 倍），验证了合成数据规模化路径；但 JointF1 持续低迷（0.005--0.011），标号-参数值配对仍是关键瓶颈；
