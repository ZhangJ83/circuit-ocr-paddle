...
合成文字图片具有完美的视觉$\to$文字映射关系。当模型面对无法通过统计模式"猜"对的输入时，唯一的降低 loss 路径是真正读取图片中的文字。这强制保持了视觉编码器活跃，防止 LLM 退化为高频 token 机械重复。20\% 的混合比例在防塌缩效果和领域适应性之间取得了平衡：比例过低则防塌缩效果不足，比例过高则可能使模型过拟合合成数据的简单视觉风格。
