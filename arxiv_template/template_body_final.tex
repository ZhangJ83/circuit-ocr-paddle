...
\begin{itemize}[nosep]
    \item \textbf{KiCad 原理图（1,200 张）}：来自 GitHub/KiCad 社区真实开源项目，\texttt{kicad-cli} 导出 SVG 后经 \texttt{cairosvg} 渲染为 PNG。GT 从 SVG stroked-text 可见文字提取。经 7 轮递进式验证（自动化校验 $\to$ 半自动清洗 $\to$ KiCad 网表交叉验证 $\to$ 人工抽检），标注准确率超过 99\%。
    \item \textbf{合成文字识别（300 张）}：PIL 程序生成，用于对抗小数据集模态塌缩。GT 由模板化自动标注生成。
    \item \textbf{真实拍照（20 张）}：手机拍摄打印件，用于评估模型对真实场景的鲁棒性。
\end{itemize}

\textbf{合成KiCad预训练数据（5,000 张）}：\texttt{kicad-cli} 程序化生成，随机组合 10 类元件符号（电阻、电容、电感、二极管、BJT、MOSFET、运放、连接器、电源符号），自动生成网表并导出 SVG 后渲染为 PNG。GT 从 SVG stroked-text 提取，与渲染图像完全对齐。该数据用于纯合成预训练实验，未混入任何真实电路图。
