...
These three bugs are not merely implementation errors---they represent generally applicable findings for the PaddleOCR-VL fine-tuning community. The causal token double-shift affects any training script that manually applies causal masking on top of \texttt{AutoModelForConditionalGeneration}, which already performs this internally. The BPE boundary merging issue applies to all sequence-generation fine-tuning workflows that concatenate prompt and label strings before tokenization. The \texttt{set\_state\_dict} silent failure is a Paddle 3.1.0 API regression that causes LoRA weights to silently not load.
...
\textbf{V10-Fixed} & \textbf{2026-07} & \textbf{Fix set\_state\_dict + all 3 bugs}, 1,554 training samples, 3 epochs & \textbf{CompF1 0.2061 (4.5$\times$), TokenRec 0.1540 (96$\times$), S600 optimal} & \textbf{Phase 1 Complete} \\
\bottomrule
\end{tabular}
\par\smallskip
...
\textbf{Three Critical Bugs (Fixed in V10).} The three bugs discovered during version iteration are technical findings with broader implications for PaddleOCR-VL fine-tuning:
\begin{enumerate}[nosep]
