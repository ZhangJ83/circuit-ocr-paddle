#!/bin/bash
# Kill old training, then run PaddleOCR-VL LoRA training (FIXED: full sequence loss)
LOGFILE=/mnt/g/mimo_project/circuit_ocr/circuit-ocr-dataset/training_run_v2.log
rm -f "$LOGFILE"
...
cat PaddleOCR-VL-LoRA-circuit-ocr/training.log 2>/dev/null | tee -a "$LOGFILE"
